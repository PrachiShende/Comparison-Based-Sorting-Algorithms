...
def sortedList(sort_techniques, timeArray_average, list_to_sort, input_sizes):
    runs = 20
    ...
    for j in range(len(sort_techniques)):
        ...
        timeArray_average[j].append(sum(timeArray[j])/runs)
        timeArray[j]=[]
    return timeArray_average

def graph_func(timeArray_average, input_sizes, title):
    plt.plot(timeArray_average[0], label = "insertionSort",c='red')
    plt.plot(timeArray_average[1], label = "mergeSort",c='green')
    plt.plot(timeArray_average[2], label = "heapSort",c='blue')
    plt.plot(timeArray_average[3], label = "quickSort",c='yellow')
    plt.plot(timeArray_average[4], label = "modifiedQuickSort",c='black')
    plt.title(title)
